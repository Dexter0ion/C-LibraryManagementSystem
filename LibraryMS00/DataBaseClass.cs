...
namespace LibraryMS00
{
    class DataBaseClass
    {
        ...
        /// Jan 23th 2017 第三次编辑 修改bug 结合功能测试 pass basically
        /// code by dexion
        ///
        /// </summary>
        /// <returns></returns>
            ...
            
        {
            ...
            {
                //创建命令对象
                SqlCommand sqlCmd = new SqlCommand(sqlCmdStr,sqlNewCnt);

                //创建操作对象
                SqlDataAdapter newDataAdapter = new SqlDataAdapter();
                newDataAdapter.SelectCommand = sqlCmd;
                //将DataSet的修改提交数据库
                SqlCommandBuilder newCmdBuilder = new SqlCommandBuilder(newDataAdapter);
                newDataAdapter.Update(DataBaseOperation(OperationType, newDataAdapter, newBook), "bookInfo");
        ...
        public DataSet DataBaseOperation(string OperationType, SqlDataAdapter newDataAdapter, BookInfo newBookInfo)
        {
            ...
            {
                if(decimal.Parse(myRow["bookId"].ToString().Trim()) == newBookInfo.Id&&OperationType!="delete")
                {
